import React, { useEffect, useReducer } from "react";

import API, { graphqlOperation } from "@aws-amplify/api";
import PubSub from "@aws-amplify/pubsub";

import { createTodo, deleteTodo } from "./graphql/mutations";
import { listTodos } from "./graphql/queries";
import { onCreateTodo, onDeleteTodo } from "./graphql/subscriptions";

import awsconfig from "./aws-exports";
import "./App.css";

API.configure(awsconfig);
PubSub.configure(awsconfig);

// Action Types
const QUERY = "QUERY";
const SUBSCRIPTION = "SUBSCRIPTION";
const DELETESUBSCRIPTION = "DELETESUBSCRIPTION";

const initialState = {
  todos: [],
};

const reducer = (state, action) => {
  switch (action.type) {
    case QUERY:
      return { ...state, todos: action.todos };
    case SUBSCRIPTION:
      return { ...state, todos: [...state.todos, action.todo] };
    case DELETESUBSCRIPTION:
      const a = state.todos.filter((todo) => todo.id !== action.todo.id);
      return { ...state, todos: a };
    default:
      return state;
  }
};
function App() {
  const [state, dispatch] = useReducer(reducer, initialState);

  async function createNewTodo() {
    const todo = { name: "test", description: "test" };
    await API.graphql(graphqlOperation(createTodo, { input: todo }));
  }

  const deletetodo = async (id) => {
    const deleteId = { id: id };
    await API.graphql(graphqlOperation(deleteTodo, { input: deleteId }));
  };

  useEffect(() => {
    async function getData() {
      const todoData = await API.graphql(graphqlOperation(listTodos));
      dispatch({ type: QUERY, todos: todoData.data.listTodos.items });
    }
    getData();

    const subscription = API.graphql(graphqlOperation(onCreateTodo)).subscribe({
      next: (eventData) => {
        const todo = eventData.value.data.onCreateTodo;
        dispatch({ type: SUBSCRIPTION, todo });
      },
    });

    const subscriptions = API.graphql(graphqlOperation(onDeleteTodo)).subscribe(
      {
        next: (eventData) => {
          console.log(eventData);
          const todo = eventData.value.data.onDeleteTodo;
          dispatch({ type: DELETESUBSCRIPTION, todo });
        },
      }
    );

    return () => subscription.unsubscribe();
  }, []);

  return (
    <div className="App">
      <button onClick={createNewTodo}>Add Todo</button>
      <div>
        {state.todos.length > 0 ? (
          state.todos.map((todo) => (
            <p key={todo.id}>
              <button onClick={() => deletetodo(todo.id)}>削除</button>
              {todo.name} : {todo.description}
            </p>
          ))
        ) : (
          <p>Add some todos!</p>
        )}
      </div>
    </div>
  );
}

export default App;
